...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      if t == 0:
        ...
        next_h, _ = rnn_step_forward(x[:, t, :], h[:, t-1, :], Wx, Wh, b)
        h[:, t, :] = next_h
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      if t == 0:
        dxt, dh0, dWxt, dWht, dbt = rnn_step_backward(dh[:, t, :] + dprev_h, (x[:, t, :],h0, Wx, Wh, b, h[:, t, :]))
      else:
        dxt, dprev_h, dWxt, dWht, dbt = rnn_step_backward(dh[:, t, :] + dprev_h, (x[:, t, :],h[:, t-1, :], Wx, Wh, b, h[:, t, :]))
      dx[:, t, :] = dxt
      dWx += dWxt
      dWh += dWht
      db += dbt
