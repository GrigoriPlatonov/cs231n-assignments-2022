...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j1 in range(2*N):
          if j1 != k:
            sum1 += torch.exp(sim(z_k, out[j1]) / tau)
        for j2 in range(2*N):
          if j2 != k+N:
            sum2 += torch.exp(sim(z_k_N, out[j2]) / tau)

        pair_sim = torch.exp(sim(z_k, z_k_N) / tau)

        total_loss +=  - (torch.log(pair_sim / sum1) + torch.log(pair_sim / sum2))
...
def sim_positive_pairs(out_left, out_right):
    ...
    # pass
    mag_left = torch.linalg.norm(out_left, dim=1)
    mag_right = torch.linalg.norm(out_right, dim=1)
    mag_mul = torch.unsqueeze(mag_left * mag_right, 1)
    pos_pairs = torch.diag(torch.matmul(out_left, out_right.T) / mag_mul).view(-1, 1)
...
def compute_sim_matrix(out):
    ...
    # pass
    mags = torch.linalg.norm(out, dim=1, keepdim=True)
    mag_pairs = torch.matmul(mags, mags.T)
    sim_matrix = torch.matmul(out, out.T) / mag_pairs
